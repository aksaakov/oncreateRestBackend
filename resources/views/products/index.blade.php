...
@section('content')
    ...
    <form action="{{ route('products.index') }}" method="get">
        <div class="row">
            ...
            <div class="col-sm-6">
                <div class="form-group">
                    <label class="control-label">{{ __('messages.products.f_vendor') }}</label>
                    <select name="filter[vendor]" class="form-control">
                        <option value=""></option>
                        @foreach(\App\Vendor::all() as $item)
                            <option value="{{ $item->id }}" @if(is_array($filter) && isset($filter['vendor']) && $filter['vendor'] == $item->id) selected @endif>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>
    ...
    <table class="table table-striped table-hover">
        <thead>
            <tr>
                ...
                <th>{{__('messages.products.f_vendor')}}</th>
